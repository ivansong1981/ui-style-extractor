...
description: Extract visual design systems from existing websites into reusable markdown style guides, optional website demos, and optional app UI style guides. Use when analyzing a live site, screenshots, or harvested CSS to document colors, typography, spacing, layout, components, motion, and design rules, or when generating new website or app UI that should match a reference site's visual language without copying its content.
---
...
3. Duplicate the blank template and fill it section by section.
...
- Use `../../../ui-style-extractor/assets/style-guide-template.md` as the blank extraction template.
- Use `../../../ui-style-extractor/assets/app-ui-style-template.md` as the blank app adaptation template.
- Read `../../../ui-style-extractor/references/extraction-workflow.md` when you need the detailed page-selection and evidence-gathering workflow.
- Read `../../../ui-style-extractor/references/app-adaptation-workflow.md` when translating a website system into an app UI guide.
- Read `../../../ui-style-extractor/references/quality-bar.md` when deciding whether a guide or demo is complete enough to ship.

## Install Note

- This wrapper exists so Claude Code can discover the shared skill when this repository is open as a project.
- For installation into another project or into `~/.claude/skills/`, copy the canonical `ui-style-extractor/` folder from this repository into that `skills/` directory.
